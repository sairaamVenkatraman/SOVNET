...
class InitialPreActResidualBlock(nn.Module):
    def __init__(self,in_channels,out_channels):
    ...
    def forward(self, x):
        out = F.selu(self.bn1(x))
        shortcut = self.shortcut(out)
        out = self.conv1(out)
        out = self.conv2(F.selu(self.bn2(out)))
        out += shortcut
        return out
...
class ReconstructionLayer(nn.Module):
  ...
  def __init__(self,num_capsules,capsule_dim,im_size,im_channels):
    ...
    self.decoder = nn.Sequential(
                                 ...
                                 nn.ConvTranspose2d(in_channels=32, out_channels=32, kernel_size=5, stride=1),
                                 nn.SELU(),
                                 nn.ConvTranspose2d(in_channels=32, out_channels=32, kernel_size=4, stride=1),
                                 nn.SELU(),
...
class GcnnSovnet(nn.Module):
  def __init__(self):
    ...
    self.initial_conv = InitialPreActResidualBlock(1,64)
    #(batch_size,64,19,19)  
    self.primary_capsules = PrimaryCapsules(num_capsules=5,in_channels=64,out_channels=64)
    #(batch_size,10,32,4,19,19) 
    self.hidden_capsules1 = ConvolutionalCapsules(num_in_capsules=5,in_capsule_dim=64,num_out_capsules=5,out_capsule_dim=64,stride=1)
    #(batch_size,10,32,4,19,19)
    self.hidden_capsules2 = ConvolutionalCapsules(num_in_capsules=5,in_capsule_dim=64,num_out_capsules=5,out_capsule_dim=64,stride=2)
    #(batch_size,10,32,4,10,10)
    self.hidden_capsules3 = ConvolutionalCapsules(num_in_capsules=5,in_capsule_dim=64,num_out_capsules=5,out_capsule_dim=64,stride=1)
    #(batch_size,10,32,4,10,10)
    self.hidden_capsules4 = ConvolutionalCapsules(num_in_capsules=5,in_capsule_dim=64,num_out_capsules=5,out_capsule_dim=64,stride=2)
    #(batch_size,10,32,4,5,5)
    self.hidden_capsules5 = ConvolutionalCapsules(num_in_capsules=5,in_capsule_dim=64,num_out_capsules=5,out_capsule_dim=64,stride=1)
    #(batch_size,10,32,4,5,5)
    self.class_capsules = ConvolutionalCapsules(num_in_capsules=5,in_capsule_dim=64,num_out_capsules=10,out_capsule_dim=64,kernel_size=5,class_=True)
    #(batch_size,10,32,4,1,1)
    self.reconstruction_layer = ReconstructionLayer(10,64,40,1)
    #(batch_size,3,64,64)
