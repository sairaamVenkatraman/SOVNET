"""
   This file trains CNN and Group equivariant CNN baselines on FashionMNIST for three types of affine transformations at train and test level  
"""
...
def main_loop():
    baseline_list = ["SimpleConvP4"]#,"SimpleConvP4M"]#list of conv baselines
    ...
    train_loaders_desc, test_loaders_desc = get_loaders_fashionmnist(train_translation_rotation_list,test_translation_rotation_list,BATCHSIZE)  
    for baseline in baseline_list:
        train_test(baseline,train_loaders_desc,test_loaders_desc) 
 
def get_loaders_fashionmnist(train_translation_rotation_list,test_translation_rotation_list,batch_size):
...
main_loop()
'''model = CnnResNet18()
checkpoint = torch.load(SAVEPATH+'CnnResNet18'+'/train_0.075_0.075_180_FashionMNISTtrain_trail_1.pth_349',map_location={'cuda:0': 'cpu'})
model.load_state_dict(checkpoint['model_state_dict'])
model.to(DEVICE)
check_point = []
torch.cuda.empty_cache()
train_translation_rotation_list = [((0.075,0.075),180)]
test_translation_rotation_list = [((0,0),0),((0.075,0.075),30),((0.075,0.075),60),((0.075,0.075),90),((0.075,0.075),180)]
train_loaders_desc, test_loaders_desc = get_loaders_fashionmnist(train_translation_rotation_list,test_translation_rotation_list,BATCHSIZE)
with torch.no_grad():
    test_accuracy_list, test_loss_list = test(model,test_loaders_desc,349,train_loaders_desc[0])
