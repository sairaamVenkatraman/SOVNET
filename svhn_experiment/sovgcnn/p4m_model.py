...
class InitialP4MResidualBlock(nn.Module):
...
class CapsuleP4MResidualBlock(nn.Module):
  '''Residual Block'''
...
class PrimaryCapsules(nn.Module):
  ...
  def __init__(self,num_capsules=10,in_channels=32,out_channels=32):
    ...
    self.capsules = nn.Sequential(
                                  ...
                                  #nn.SELU(),
                                  #nn.BatchNorm3d(out_channels),  
                                  CapsuleP4MResidualBlock(num_capsules,out_channels,out_channels,stride=1)
                                 )   
  ...
  def forward(self, x):
    ...
    output = output.view(-1,self.num_capsules,self.out_channels,8,H,W)
...
class ConvolutionalCapsules(nn.Module):
  ...
  def __init__(self,num_in_capsules,in_capsule_dim,num_out_capsules,out_capsule_dim,kernel_size=7,stride=1,padding=0,class_=False):
    ...
    if class_ == False:
       self.projection_network = CapsuleP4MResidualBlock(num_out_capsules,in_capsule_dim,out_capsule_dim,stride)
    else:
       self.projection_network = nn.Sequential(P4MConvP4M(in_capsule_dim,out_capsule_dim*num_out_capsules,kernel_size,stride,padding))  
...
class ReconstructionLayer(nn.Module):
  ...
  def __init__(self,num_capsules,capsule_dim,im_size,im_channels):
    ...
    self.decoder = nn.Sequential(
                                 ...
                                 nn.ConvTranspose2d(in_channels=32, out_channels=32, kernel_size=5, stride=1),
                                 nn.SELU(),
                                 nn.ConvTranspose2d(in_channels=32, out_channels=3, kernel_size=8, stride=1),
                                 #nn.SELU(),
...
class GcnnSovnet(nn.Module):
  def __init__(self):
    ...
    self.initial_conv = InitialP4MResidualBlock(3,64)
    #(batch_size,32,28,28)  
    ...
    self.reconstruction_layer = ReconstructionLayer(10,64,32,3)
    #(batch_size,3,64,64)
