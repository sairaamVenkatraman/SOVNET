...
from p4m_model import *


DATAPATH = "../../data/SVHN/"
DEVICE = torch.device("cuda:0" if torch.cuda.is_available else "cpu")
SAVEPATH = "./check_points/SVHN/"
BATCHSIZE = 64
...
def train_test(baseline,num_epochs,train_loaders_desc,test_loaders_desc):
    for (train_loader, train_loader_desc) in train_loaders_desc:
        ...
        exp_lr_scheduler = lr_scheduler.LambdaLR(optimizer,lambda epoch: 0.96**epoch)#lambda epoch: 0.5**(epoch // 10))
...
def main_loop():
    ...
    num_epochs = 150
    train_translation_rotation_list = [((0.2,0.2),0)]
    test_translation_rotation_list = [((0,0),0)]
    train_loaders_desc, test_loaders_desc = get_loaders_svhn(train_translation_rotation_list,test_translation_rotation_list,BATCHSIZE)  
