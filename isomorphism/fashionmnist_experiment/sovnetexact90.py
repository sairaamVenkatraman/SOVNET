...
class ReconstructionLayer(nn.Module):
  ...
  def __init__(self,num_capsules,capsule_dim,im_size,im_channels):
    ...
    self.decoder = nn.Sequential(
                                 ...
                                 nn.ConvTranspose2d(in_channels=32, out_channels=32, kernel_size=5, stride=1),
                                 nn.SELU(),
                                 nn.ConvTranspose2d(in_channels=32, out_channels=1, kernel_size=5, stride=1),
                                 #nn.SELU(),
...
class GcnnSovnet(nn.Module):
  def __init__(self):
    ...
    self.hidden_capsules2 = ConvolutionalCapsules(num_in_capsules=5,in_capsule_dim=64,num_out_capsules=5,out_capsule_dim=64,kernel_size=3)
    #(batch_size,10,32,4,13,13)
    ...
    #(batch_size,10,32,4,9,9)
    self.hidden_capsules4 = ConvolutionalCapsules(num_in_capsules=5,in_capsule_dim=64,num_out_capsules=5,out_capsule_dim=64,kernel_size=3)
    ...
    self.reconstruction_layer = ReconstructionLayer(10,64,29,1)
    #(batch_size,3,64,64)

  def forward(self,x,target):
      conv_output = self.initial_conv(x)
      primary_capsules = self.primary_capsules(conv_output)
      hidden_capsules_one, degree_score_one = self.hidden_capsules1(primary_capsules,routing_iterations=1)
      hidden_capsules_two, degree_score_two = self.hidden_capsules2(hidden_capsules_one,routing_iterations=2)
      hidden_capsules_three, degree_score_three = self.hidden_capsules3(hidden_capsules_two,routing_iterations=2)
      hidden_capsules_four, degree_score_four = self.hidden_capsules4(hidden_capsules_three,routing_iterations=1)
      hidden_capsules_five, degree_score_five = self.hidden_capsules5(hidden_capsules_four,routing_iterations=1)
      class_capsules, degree_score_class = self.class_capsules(hidden_capsules_five,routing_iterations=3)
      ...
      degree_score = [degree_score_one, degree_score_two, degree_score_three, degree_score_four, degree_score_five, degree_score_class]
      return capsules, degree_score     
...
checkpoint_file = ''#PLEASE FILL OUT WITH CHECKPOINT AFTER TRAINING CODE IN SAME FOLDER
...
check_point = [] 
no_rotation = transforms.Compose([transforms.Resize(29),transforms.ToTensor(),transforms.Normalize((0.5,),(0.5,))])
rotation = transforms.Compose([transforms.Resize(29),transforms.RandomRotation((180,180)),transforms.ToTensor(),transforms.Normalize((0.5,), (0.5,))])
test_dataset1 = FashionMNIST('./',train=False,download=True,transform=no_rotation)
test_dataset2 = FashionMNIST('./',train=False,download=True,transform=rotation)
